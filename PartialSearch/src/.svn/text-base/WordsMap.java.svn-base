...
public class WordsMap
{
	...
	/* For each query word, it finds words in the inverted index that start with this query word. Then it either creates a new SearchResult or 
	 * updates the existing SearchResult depending on whether the corresponding file path was previously added to locationsMap or not. 
	 * 
	 */
	public ArrayList<SearchResult> partialSearch(String[] input)
	{
		/* The local hash map i.e locationsMap is where file path is stored as key and SearchResult object as value. 
		 * 
		 */
				...
				word = word.toLowerCase();
		 		...
		 		for(String key: outerTailMap.keySet())
		 		{
		 			if(key.startsWith(word))
		 			{
						...
						{
							ArrayList<Integer> aList = innerTailMap.get(filePath);
