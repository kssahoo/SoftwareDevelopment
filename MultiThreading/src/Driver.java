...
public class Driver 
{
	 ...
	 * operation on each query line and outputs the result to a text file. 
	 * For multi-threading, main method takes a value which is treated as required 
	 * number of threads for the project.   
	 */
	public static void main(String[] args) 
	{
		...
		if ((ap.hasFlag("-t")) && (ap.hasValue("-t"))) 
		{
			...
			if (numThreads > 0) 
			{
				...
				MultiThreadedDirectoryTraverser mDt = new MultiThreadedDirectoryTraverser(tSi, minions);
				...
				if (ap.hasFlag("-q")) 
				{
					MultiThreadedIndexSearcher mIs = new MultiThreadedIndexSearcher(tSi, minions);
